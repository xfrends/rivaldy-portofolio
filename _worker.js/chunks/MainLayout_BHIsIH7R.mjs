...
const $$Footer = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<footer class="py-10 border-t border-gray-100 mt-auto"> <div class="container mx-auto"> <div class="flex flex-col md:flex-row justify-between items-center"> <div class="text-sm font-sans text-gray-500 mb-4 md:mb-0">
© ${year} <span class="font-brand font-semibold">${owner}</span>. All rights reserved.
</div> <div class="flex space-x-6"> ${socialLinks.map((socialLink) => renderTemplate`${renderComponent($$result, "SocialIcon", $$SocialIcon, { "socialLink": socialLink })}`)} </div> </div> </div> </footer>`;
...
const $$NavBar = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@400..700&display=swap" rel="stylesheet">${maybeRenderHead()}<nav x-data="{ isOpen: false, scrolled: false }" @scroll.window="scrolled = window.scrollY > 50" class="fixed top-0 left-0 w-full z-50 transition-all duration-300" :class="scrolled ? 'py-3 bg-white shadow-sm' : 'py-5 bg-transparent'"> <div class="container mx-auto flex justify-between items-center"> <a${addAttribute(withBase(""), "href")} id="logo" class="text-2xl md:text-4xl font-semibold tracking-tight"> ${title} </a> <!-- Desktop Menu --> <div class="hidden md:flex space-x-8"> ${menuItems.map((item) => renderTemplate`<a${addAttribute(item.link, "href")} class="nav-link relative after:content-[''] after:absolute after:left-0 after:bottom-[-2px] after:h-[2px] after:bg-black after:w-0 hover:after:w-full transition-all"> ${item.name} </a>`)} </div> <!-- Mobile Menu Button --> <button @click="isOpen = !isOpen" class="md:hidden text-gray-800 focus:outline-none"> <svg x-show="!isOpen" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path> </svg> <svg x-show="isOpen" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path> </svg> </button> </div> <!-- Mobile Menu --> <div x-show="isOpen" class="md:hidden absolute top-full left-0 w-full bg-white shadow-md"> <div class="flex flex-col py-4 px-6 space-y-4"> ${menuItems.map((item) => renderTemplate`<a${addAttribute(item.link, "href")} class="text-lg" @click="isOpen = false"> ${item.name} </a>`)} </div> </div> </nav>`;
